...
#include "password_policy.h"  // EKLENEN INCLUDE
#include <WebServer.h>
#include <ArduinoJson.h>      // EKLENEN INCLUDE

extern Settings settings;
extern WebServer server;
extern PasswordPolicy passwordPolicy;  // EKLENEN EXTERN
...
void handleUserLogin() {
    ...
    if (u == settings.username) {
        ...
        if (hashedAttempt == settings.passwordHash) {
            ...
            // BASİT JSON response (String concatenation ile)
            String response = "{";
            ...
            if (mustChange) {
                ...
                if (passwordPolicy.isDefaultPassword) {
                    response += ",\"reason\":\"İlk girişinizde varsayılan parolayı değiştirmeniz gerekmektedir.\"";
                    addLog("Sebep: Varsayılan parola", INFO, "AUTH");
                } else if (isPasswordExpired()) {
                    response += ",\"reason\":\"Parolanızın süresi dolmuştur.\"";
